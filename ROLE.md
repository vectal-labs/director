...
2. Select a candidate using the operator's profile and the scan evidence. Read enough of its conversation to understand the requested task, current state, and remaining work. Read its repo's `AGENTS.md` and relevant decisions. If context is insufficient, keep reading or report the uncertainty.
3. Assess the candidate using the profile. Distinguish finished work, an actionable blocker, and a decision that needs the operator. Present the evidence, judgment, and exact proposed action using the operator's preferred style.
